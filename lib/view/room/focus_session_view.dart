...
class _FocusSessionViewState extends State<FocusSessionView> {
  ...
  // Total time spent paused during this session -> counted as Break Time.
  int pausedSeconds = 0;
  ...
  // Saves the actual minutes studied (or paused) so far to Firestore, so
  // both a full session and an early "Leave" still count toward Progress.
  void _saveSession(int minutesStudied, {String type = 'focus'}) {
    debugPrint('Trying to save $type: $minutesStudied minute(s).');

    if (minutesStudied <= 0) {
      debugPrint('Not saved: less than 1 minute.');
      return;
    }
    ...
    if (user == null) {
      debugPrint(
        'Session NOT saved: no logged-in Firebase user.',
      );
      return;
    }
    ...
    FirestoreService()
      ...
      category: widget.category,
        ...
        .then((_) {
      debugPrint('$type saved to Firestore.');
    }).catchError((error) {
      debugPrint('$type save FAILED: $error');
    });
  }
  ...
  void _openSessionComplete() {
    ...
    // Full session finished normally -> save the full planned duration.
    _saveSession(widget.durationMinutes);

    // Save any time spent paused as Break Time.
    _saveSession(
      (pausedSeconds / 60).ceil(),
      type: 'break',
    );
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                ...
                final secondsStudied =
                    totalSeconds - remainingSeconds;

                final minutesStudied =
                    (secondsStudied / 60).ceil();

                _saveSession(minutesStudied);
